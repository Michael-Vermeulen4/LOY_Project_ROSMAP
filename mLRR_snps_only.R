  ...
  # par - mLRR for PAR snps only
    ...
    for(j in c(1:22,"X","Y","PAR")){
      if(j=="PAR"){
        ...
        subset(snps, snps$Name %in% par_snps$V1) -> tmp
        message(name)
...
write.table(file="/zfs3/scratch/saram_lab/ROSMAP_CEL/ROSMAP_Cel/gw6/apt/DEC6_apt/mLRR_SNP_ONLY_JAN9.txt", x = output, quote=FALSE, row.names=TRUE, sep="\t")
